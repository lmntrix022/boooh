...
/**
 * Template Premium — Luxe charcoal
 * Typographie raffinée, carte client crème, double ligne, espace généreux
 */
export class PremiumPdfTemplate extends BasePdfTemplate {
  async generate(doc: any, invoice: Invoice, settings: InvoiceSettings | null): Promise<void> {
    ...
    const accent = [51, 65, 85];
    const cream = [252, 251, 249];
    ...
    this.drawInvoiceTable(doc, invoice, yPosition, margin, pageWidth, pageHeight, cream, charcoal, accent);
    await this.drawFooter(doc, invoice, settings, pageWidth, pageHeight, margin, charcoal, accent);
  }
}
