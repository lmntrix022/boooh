...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!amount || !customer_email || !success_url || !cancel_url) {
      return new Response(
        JSON.stringify({
    ...
    const amountInSmallestUnit = amount >= 10000
      ? Math.round(amount) // Déjà en centimes
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: `${order_type === 'physical' ? 'Commande physique' : 'Commande digitale'}`,
    ...
    console.log(`✅ Stripe Checkout session créée: ${session.id} pour la commande ${order_id}`);
    ...
    console.error("❌ Erreur dans stripe-create-checkout:", error);
    return new Response(
      JSON.stringify({
