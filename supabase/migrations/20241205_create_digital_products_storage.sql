-- Migration pour créer le bucket de stockage des produits numériques
-- Date: 2024-12-05
...
-- 2. Créer le bucket thumbnails pour les images de couverture
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
  'digital-thumbnails',
  'digital-thumbnails',
...
-- 5. Fonction pour générer des URLs sécurisées pour les téléchargements
...
-- 6. Fonction pour nettoyer les fichiers orphelins
...
-- 7. Trigger pour nettoyer les fichiers lors de la suppression d'un produit
...
-- 9. Fonction pour vérifier les limites de stockage
...
-- 10. Index pour optimiser les requêtes de stockage
...
-- 11. Vue pour les statistiques de stockage par utilisateur
...
FROM storage.objects 
WHERE bucket_id IN ('digital-products', 'digital-thumbnails')
GROUP BY split_part(name, '/', 1), bucket_id;

-- 12. Fonction pour migrer les fichiers existants (si nécessaire)
CREATE OR REPLACE FUNCTION migrate_existing_files()
RETURNS void AS $$
BEGIN
  -- Cette fonction peut être utilisée pour migrer des fichiers existants
  -- vers la nouvelle structure de dossiers
  RAISE NOTICE 'Migration des fichiers existants...';
