...
import { BasePdfTemplate } from './pdfTemplates/basePdfTemplate';

/**
 * Service de génération de bons de commande
 * Utilise les mêmes templates que les factures
 */
export class PurchaseOrderService {
  /**
   * Génère un bon de commande PDF avec le design du template sélectionné
   */
  static async generatePurchaseOrderPDF(
    ...
    // Couleurs cohérentes (bleu pour bon de commande)
    ...
    const accentColor: [number, number, number] = [37, 99, 235]; // Bleu foncé

    // En-tête professionnel
    this.drawHeader(doc, pageWidth, primaryColor, 'BON DE COMMANDE', `BC-${order.id?.substring(0, 8).toUpperCase()}`);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...darkGray);
    doc.text('POUR (CLIENT)', col2X, clientY);
    ...
    doc.setTextColor(...darkGray);
    yPosition += 3;
    doc.text('Date du bon', margin + 5, yPosition);
    doc.text('Date de livraison souhaitée', col2X, yPosition);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(...mediumGray);
    yPosition += 4;
    doc.text(new Date(order.created_at).toLocaleDateString('fr-FR'), margin + 5, yPosition);
    doc.text(new Date(order.created_at).toLocaleDateString('fr-FR'), col2X, yPosition);
    ...
    yPosition = this.drawOrderTable(doc, order, yPosition, margin, pageWidth, lightGray, darkGray, primaryColor, settings);
  ...
  /**
   * Dessine le tableau des articles et retourne la nouvelle position Y
   */
  private static drawOrderTable(
    doc: any,
    order: OrderWithProduct,
    startY: number,
    margin: number,
    ...
    const columnWidths = [70, 30, 30, 30]; // Description, Qty, Prix unitaire, Total
    ...
    const headers = ['Description', 'Qty', 'Prix unitaire', 'Total'];
    ...
    const nameLines = doc.splitTextToSize(productName, columnWidths[0] - 6);
    doc.text(nameLines, xPosition + 3, yPosition + 4);
    
    xPosition += columnWidths[0];
    doc.text((order.quantity ?? 1).toString(), xPosition + 3, yPosition + 4, { align: 'right' });
    
    xPosition += columnWidths[1];
    const unitPrice = typeof order.products?.price === 'string' 
      ? parseFloat(order.products.price) 
      : (order.products?.price || 0);
    doc.text(`${unitPrice.toLocaleString('fr-FR')} F`, xPosition + 3, yPosition + 4, { align: 'right' });
    
    xPosition += columnWidths[2];
    const total = (order.quantity || 1) * unitPrice;
    doc.text(`${total.toLocaleString('fr-FR')} F`, xPosition + 3, yPosition + 4, { align: 'right' });
    ...
    doc.setFontSize(9);
    doc.setTextColor(...textColor);
    doc.text('TVA (18%):', totalX, yPosition);
    ...
    doc.setFontSize(11);
    doc.setFillColor(...primaryColor);
    doc.rect(totalX - 5, yPosition - 4, 60, 8, 'F');
    doc.setTextColor(255, 255, 255);
    ...
    const conditions = [
      ...
      '• Conditions de paiement: à confirmer',
